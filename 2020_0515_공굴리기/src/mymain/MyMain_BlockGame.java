...
public class MyMain_BlockGame extends JFrame {
	...
	private void init_block() {
		...
		for(int i=0;i<block_array.length;i++) {//행수
			for(int k=0;k<block_array[i].length;k++) {//열수
				...
				block.pos.x = k*block_w; //k = 0 1 2 3 4 5 6
				block.pos.y = i*block_h; //i = 0 1 2 3 4 
		...
		this.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if(key==KeyEvent.VK_S) {
					//                  행수 * 열수
					block_count = block_array.length *
							      block_array[0].length; 
					//if(timer!=null)
					timer.start();
	...
	private void collision_block_right() {
		...
		//블럭이 없는 영역에서는 체크종료해라..
		if(cy >= block_h * block_array.length )return;
